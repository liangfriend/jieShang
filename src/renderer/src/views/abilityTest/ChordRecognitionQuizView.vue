...
import {
  CHORD_ALL_QUALITIES,
  formatChordChoiceLabel,
  type ChordQuality
} from '@renderer/constant/chordRecognitionQuiz'
import { useChordRecognitionQuiz } from '@renderer/views/abilityTest/useChordRecognitionQuiz'

defineOptions({ name: 'ChordRecognitionQuizView' })
...
const {
  ...
  playAgain,
  backToSetup
} = useChordRecognitionQuiz()

const qualityLabel = (quality: ChordQuality) => t(`chordRecognitionQuiz.qualities.${quality}`)

const labeledChoices = computed(() =>
  choices.value.map((choice) => ({
    ...choice,
    label: formatChordChoiceLabel(choice, qualityLabel)
  }))
)
...
function isQualityEnabled(quality: ChordQuality) {
  return enabledQualities.value.includes(quality)
}

function choiceClass(id: string) {
  if (phase.value !== 'result' || !lastRoundResult.value) return {}
  const correctId = lastRoundResult.value.chordId
  const selected = selectedId.value
  return {
    'choice--correct': id === correctId,
    'choice--wrong': selected === id && id !== correctId
  }
}
</script>
...
<template>
  <div class="quiz-page">
    <header class="quiz-page__header">
      ...
      <h1 class="quiz-page__title">{{ t('chordRecognitionQuiz.title') }}</h1>
    ...
    <main class="quiz-page__main">
      <section v-if="phase === 'setup'" class="quiz-panel">
        <h2 class="quiz-panel__heading">{{ t('chordRecognitionQuiz.setup.heading') }}</h2>

        <label class="quiz-field">
          <span class="quiz-field__label">{{ t('chordRecognitionQuiz.setup.qualities') }}</span>
          <div class="quiz-field__options">
            <button
              v-for="quality in CHORD_ALL_QUALITIES"
              :key="quality"
              type="button"
              class="chip"
              :class="{ 'chip--active': isQualityEnabled(quality) }"
              @click="toggleQuality(quality)"
            >
              {{ t(`chordRecognitionQuiz.qualities.${quality}`) }}
            </button>
          </div>
        </label>

        <label class="quiz-field">
          <span class="quiz-field__label">{{ t('chordRecognitionQuiz.setup.soundRange') }}</span>
          <div class="quiz-field__options">
            <button
              type="button"
              class="chip"
            ...
            >
              {{ t('chordRecognitionQuiz.setup.soundRangeOptions.standard') }}
            </button>
            <button
              type="button"
              class="chip"
            ...
            >
              {{ t('chordRecognitionQuiz.setup.soundRangeOptions.full') }}
            </button>
          </div>
        </label>

        <label class="quiz-field">
          <span class="quiz-field__label">{{ t('chordRecognitionQuiz.setup.showPiano') }}</span>
          <el-switch :model-value="showPiano" @update:model-value="setShowPiano" />
        </label>

        <label class="quiz-field">
          <span class="quiz-field__label">{{ t('chordRecognitionQuiz.setup.rounds') }}</span>
          <el-input-number
        ...
        <label class="quiz-field">
          <span class="quiz-field__label">{{ t('chordRecognitionQuiz.setup.countdown') }}</span>
          <el-input-number
        ...
        <button type="button" class="primary-btn" @click="startTest">
          {{ t('chordRecognitionQuiz.setup.start') }}
        </button>
      </section>
      ...
      <section v-else-if="phase === 'question' || phase === 'result'" class="quiz-stage">
        <button
          type="button"
          class="quiz-ring quiz-ring--button"
          :class="{
            'quiz-ring--correct': ringStatus === 'correct',
            'quiz-ring--wrong': ringStatus === 'wrong'
          }"
          :disabled="phase !== 'question'"
          :aria-label="t('chordRecognitionQuiz.question.replayAria')"
          @click="replaySound"
        >
          <template v-if="phase === 'question'">
            <span class="quiz-ring__countdown">{{ countdownLeft }}</span>
            <span class="quiz-ring__hint">{{ t('chordRecognitionQuiz.question.replayHint') }}</span>
          </template>
          <template v-else>
            <span class="quiz-ring__result">
              {{
                lastRoundResult?.correct
                  ? t('chordRecognitionQuiz.result.correct')
                  : lastRoundResult?.timedOut
                    ? t('chordRecognitionQuiz.result.timeout')
                    : t('chordRecognitionQuiz.result.wrong')
              }}
            </span>
          </template>
        </button>
        ...
        <div class="choice-grid">
          <button
            v-for="choice in labeledChoices"
            :key="choice.id"
            type="button"
            class="choice"
        ...
        <button
          ...
          @click="backToSetup"
        >
          {{ t('chordRecognitionQuiz.question.abort') }}
        </button>
      </section>

      <section v-else class="quiz-panel">
        <h2 class="quiz-panel__heading">{{ t('chordRecognitionQuiz.final.heading') }}</h2>
        <p class="result-value">
          {{
            t('chordRecognitionQuiz.final.accuracy', {
        ...
        <ul class="final-stats">
          <li>
            {{
              t('chordRecognitionQuiz.final.counts', {
          ...
          <li>
            {{
              t('chordRecognitionQuiz.final.avgResponse', { ms: formatMs(summary.avgResponseMs) })
            }}
          </li>
          <li>
            {{ t('chordRecognitionQuiz.final.bestStreak', { count: summary.bestStreak }) }}
          </li>
          <li v-for="item in qualitySummary" :key="item.quality">
            {{
              t('chordRecognitionQuiz.final.qualityAccuracy', {
                quality: t(`chordRecognitionQuiz.qualities.${item.quality}`),
                percent: formatPercent(item.accuracyPercent)
              })
            }}
          </li>
        </ul>
        <div class="final-actions">
          <button type="button" class="primary-btn" @click="playAgain">
            {{ t('chordRecognitionQuiz.final.playAgain') }}
          </button>
          <button type="button" class="ghost-btn" @click="backToSetup">
            {{ t('chordRecognitionQuiz.final.reset') }}
          </button>
        </div>
      </section>
    </main>
